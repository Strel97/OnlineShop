 ...
 * The API model decouples api implementation from configuration or user, so the only class tied to the implementation
...
public interface APIFactory<T extends API> {
     ...
     * only methods exposed in the T-interface are accessible.
